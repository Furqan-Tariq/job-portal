    ...
    <Card className="overflow-hidden shadow-sm hover:shadow-md transition-shadow h-full flex flex-col">
      ...
      {employer.imageUrl ? (
        <div className="relative h-40 w-full overflow-hidden">
          <Image src={employer.imageUrl || "/placeholder.svg"} alt={employer.name} fill className="object-cover" />
        </div>
      ) : (
      ...
      {/* Card content */}
      <CardContent className={`p-5 flex flex-col gap-3 flex-1 ${employer.isHighlighted ? "bg-yellow-50" : "bg-white"}`}>
        {/* Employer name */}
        <h2 className="font-semibold text-lg text-gray-900 leading-snug mb-1">{employer.name}</h2>
